...
public class JumpIfFalseCommand extends Command {
    public CommandResult execute(CommandInput commandInput) {
        ...
        if (commandInput.debug()) {
            System.out.println("At " + commandInput.instructionPointer() + ": JumpIfFalseCommand: " + commandInput.memory().get(commandInput.instructionPointer()));
